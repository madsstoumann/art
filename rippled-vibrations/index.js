...
GUI.addEventListener('gui-input', (event) => common.handleGuiEvent(event, svg, GUI, storageKey, drawWavy));
...
function drawWavy(svg, controls) {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    const path = common.createLinePath(`line${i}`, 0, pathAmplitude, lineWidth + strokeWidth, frequency, height);
