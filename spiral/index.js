...
common.commonConfig(GUI, '#323f67');
GUI.addEventListener('gui-input', (event) => common.handleGuiEvent(event, svg, GUI, storageKey, spiral));
...
function spiral(svg, controls) {
	...
	const circles = points.map((pos, index) => {
		const fillColor = common.interpolateColor(startcolor, endcolor, pos.t);
