...
GUI.addEventListener('gui-input', (event) => common.handleGuiEvent(event, svg, GUI, storageKey, creepyCrawlers));
common.init(GUI, storageKey, []);

/* === MAIN FUNCTION === */

function creepyCrawlers(svg, controls) {
	...
	const strokes = () => {
		let output = '';
		if (randomColor) {
			output += `stroke="hsl(${common.random(0, 360)}, ${common.random(50, 100)}%, ${common.random(30, 60)}%)"`;
		}
		if (randomLine) {
			output += ` stroke-width="${common.random(0.01, 0.4)}"`;
		}
		return output;
	}
