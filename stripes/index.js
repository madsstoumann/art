...
GUI.addEventListener('gui-input', (event) => common.handleGuiEvent(event, svg, GUI, storageKey, stripes));
...
function stripes(svg, controls) {
  ...
  const [H, _S, _L] = common.hexToHSL(controls.hue.value);
  ...
  const stripesMarkup = stripeWidths.map((stripeWidth, index) => {
    ...
    const fillColor = usePalette 
      ...
      : `hsl(${H}, ${common.random(50, 100)}%, ${common.random(30, 90)}%)`;
